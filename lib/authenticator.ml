...
(* XXX
   ...
   * *)
let chain_of_trust_ex ~time ?crls ?allow_ca_cert ?(allowed_hashes = Validation.sha2) cas =
  let revoked = match crls with
    | None -> None
    | Some crls -> Some (Crl.is_revoked crls ~allowed_hashes)
  in
  fun ?ip ~host certificates ->
    Validation.verify_chain_of_trust ?ip ~host ~time ?allow_ca_cert ?revoked ~allowed_hashes
      ~anchors:cas certificates

let chain_of_trust ~time ?crls ?(allowed_hashes = Validation.sha2) cas =
  chain_of_trust_ex ~time ?crls ~allowed_hashes cas
